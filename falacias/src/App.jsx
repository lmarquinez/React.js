...
function App() {
    ...
    return (
        <div>
            ...
            <ul>
                {
                    questionFalacys.map((s, i) => <LI key={i} isRed={falacy.type === s.type}><button onClick={() => evaluateAnswer(s)}>X</button> {s.type}  <ClueButton numQuestion={numQuestion} clue={s.def} />
                    </LI>)
                }
            </ul>
        </div>
